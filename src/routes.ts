...
async function getPrayerTimings(address: string, date?: string, school: number = 1, method?: string) {
  try {
...
routes.get("/qibla", async (req, res) => {
  try {
    ...
    res.status(500).send("Error fetching Qibla direction");
  }
});

routes.get("/timings/", async (req, res) => {
  try {
    const { address, date, school = 1, method } = req.query;
    const response = await getPrayerTimings(address.toString(), date ? date.toString() : null, parseInt(school.toString()), method ? method.toString() : null);
